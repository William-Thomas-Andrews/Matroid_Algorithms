...
BipartiteEdge::BipartiteEdge() {}
BipartiteEdge::BipartiteEdge(Vertex v1, Vertex v2, Weight w, Partition p1, Partition p2) : v(v1 > v2 ? v1 : v2), u(v1 > v2 ? v2 : v1), weight(w) {
...
Partition BipartiteEdge::get_left_partition() { return v_partition; }
Partition BipartiteEdge::get_right_partition() { return u_partition; }
Weight BipartiteEdge::get_weight() {
    return weight;
}
void BipartiteEdge::set_left_partition(Partition p1) {
    if (p1 != 1 or p1 != 2) { throw std::invalid_argument("The partition specified is not 1 or 2. Bipartite graphs only have 2 partitions."); }
    v_partition = p1;
}
void BipartiteEdge::set_right_partition(Partition p2) {
    if (p2 != 1 or p2 != 2) { throw std::invalid_argument("The partition specified is not 1 or 2. Bipartite graphs only have 2 partitions."); }
    u_partition = p2;
}
Vertex BipartiteEdge::get_left() { return u; }
Vertex BipartiteEdge::get_right() { return v; }
std::string BipartiteEdge::get_string() {
...
Vertex BipartiteEdge::other(Vertex t) {
    if (v == t) {
        return u;
    }
    else if (u == t) {
...
// Comparison operator <
bool BipartiteEdge::operator<(BipartiteEdge e2) {
...
BipartiteGraph::BipartiteGraph(int size_1, int size_2) : union_set(UnionFind(size_1 + size_2)) {}
BipartiteGraph::BipartiteGraph(std::vector<std::tuple<Vertex, Vertex, Weight, Partition, Partition>> input_data) : union_set(UnionFind(input_data.size())), partition_map(std::vector<int>({-1})) {
...
BipartiteGraph::BipartiteGraph(std::vector<BipartiteEdge> input_data) : union_set(UnionFind(input_data.size())) {
    edges.resize(input_data.size());
...
std::string BipartiteGraph::get_string() {
    std::string str = "";
    int count = 0;
...
std::vector<Vertex>& BipartiteGraph::get_vertices() {
    return vertices;
}


// Matroid functions begin --------------------------------------------------------------------------------------------------
void BipartiteGraph::min_sort() {
    std::sort(vertices.begin(), vertices.end(), MinCompare<Vertex>{});
}

void BipartiteGraph::max_sort() {
    std::sort(vertices.begin(), vertices.end(), MaxCompare<Vertex>{});
}

bool BipartiteGraph::not_empty() {
    return (!vertices.empty());
}

Vertex BipartiteGraph::top() {
...
bool BipartiteGraph::is_independent(Vertex& v) {
...
void BipartiteGraph::add_element(Vertex v) {
    vertices.push_back(v);
}

void BipartiteGraph::remove_element() {
    vertices.pop_back();
}

void BipartiteGraph::pop() {
    vertices.pop_back();
}
// Matroid functions end ---------------------------------------------------------------------------------------------------------


bool BipartiteGraph::in(Vertex v) {
...
std::string BipartiteGraph::get_vertices_string() {
    std::string str = "";
    for (int i = 0; i < vertices.size(); i++) {
