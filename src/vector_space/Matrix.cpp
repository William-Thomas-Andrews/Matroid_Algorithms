...
Matrix::Matrix() : rows(0), columns(0) {} // Empty Matrix
Matrix::Matrix(int r, int c) : rows(r), columns(c) { // Random data
    double min = 0; 
    double max = 10;
...
Matrix::Matrix(double item, int r, int c) : rows(r), columns(c) { // Uniform data
    for (int i = 0; i < c; i++) {
        data.push_back(Vector(r, item));
    }
    // row_reduce(*this); // Can be enabled for auto row-reduction
}
Matrix::Matrix(std::vector<Vector> input_data, int r, int c) : rows(r), columns(c) { 
...
Matrix::Matrix(const std::vector<std::vector<double>> input_data, int r, int c) : rows(r), columns(c) { 
...
Matrix::Matrix(const std::vector<std::vector<double>> input_data, int r, int c, bool by_column) : rows(r), columns(c) { 
    if (by_column) {
        if (input_data.size() != c) throw std::invalid_argument("Number of columns of array does not match dimension sizes.");
        for (int col = 0; col < c; col++) {
            if (input_data[col].size() != r) throw std::invalid_argument("Number of rows of array does not match dimension sizes.");
...
// Copy Constructor
Matrix::Matrix(const Matrix& A) : rows(A.rows), columns(A.columns), data(A.data) {
    // row_reduce(*this); // Can be enabled for auto row-reduction
}
// Destructor
Matrix::~Matrix() {}

void Matrix::print() { std::cout << get_matrix_string(); }

void Matrix::update_zeros() {
    for (int col = 0; col < columns; col++) {
...
void Matrix::add_element(Vector v) {
    if (v.is_zero()) return;
    if (rows > v.dim()) {
        while (v.dim() < rows) {
            v.add(0);
...
void Matrix::swap_vectors(Vector& v1, Vector& v2) {
    Vector temp = v2;
...
// Operators
Vector Matrix::operator()(int col_index) { // Index operator, returns a new vector of the same data (still a matrix)
...
std::vector<Vector> Matrix::get_data() { // a copy of the data
...
std::string Matrix::get_list_string() { // a copy of the data in a string format (originally arithmetic data)
    std::string str = "(";
...
std::string Matrix::to_string_precise(double value, int precision = 5) {
    std::ostringstream out;
    out << std::fixed << std::setprecision(precision) << value;
    return out.str();
}

std::string Matrix::get_matrix_string() { // a copy of the data in a string format (originally arithmetic data)
...
std::string Matrix::get_truncated_matrix_string() { // a copy of the data in a string format (originally arithmetic data)
    if (rows == 0 or columns == 0) return "[]";
    std::string str = "";
    for (int i = 0; i < rows; i++) {
        str.append("[ ");
...
bool Matrix::is_independent(Vector& v) {
...
void Matrix::multiply_row(int row, double scalar) {
    for (int col = 0; col < columns; col++) {
        this->get_element(row, col) = this->get_element(row, col) * scalar;
    }
}

void Matrix::row_replacement(int root_row, int replacee, double scalar) {
    for (int col = 0; col < columns; col++) {
